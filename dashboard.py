...
if not filtered_df.empty:
    ...
    with tab3:
        ...
        if 'selected_player' not in st.session_state:
            st.session_state.selected_player = player_options[0] if player_options else None
        
        # Check if current selected player is still in filtered options
        if st.session_state.selected_player not in player_options:
            st.session_state.selected_player = player_options[0] if player_options else None
        
        # Simple player selection with session state
        selected_player = st.selectbox(
            "Select a player to analyze:",
            options=player_options,
            index=player_options.index(st.session_state.selected_player) if st.session_state.selected_player in player_options else 0,
            key='player_selector'
        )
