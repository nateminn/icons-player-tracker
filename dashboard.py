...
if not filtered_df.empty:
    ...
    with tab3:
            ...
            st.markdown("---")
             ...
             # SEARCH PERFORMANCE METRICS - WITH GREY BOXES
    ...
    # Add this as the new tab6 content
    with tab6:
        ...
        # Load player details and calculate scores
        if player_dict:
            # Convert to DataFrame
            players_df = pd.DataFrame(player_dict.values())
            
            # Get Google search volumes from main data
            player_search_totals = df.groupby('actual_player')['july_2025_volume'].sum().to_dict()
            
            # Calculate opportunity scores
            players_df['google_search_volume'] = players_df['name'].map(player_search_totals).fillna(0)
            players_df['opportunity_score'] = players_df.apply(
                lambda row: calculate_opportunity_score(row.to_dict(), row['google_search_volume']), 
                axis=1
            )
            
            # Sort by opportunity score
            players_df = players_df.sort_values('opportunity_score', ascending=False)
            ...
            with col2:
                avg_score = players_df['opportunity_score'].mean()
                st.metric("Avg Opportunity Score", f"{avg_score:.2f}")
            with col3:
                complete_data = players_df[players_df['instagram_followers'] != 'N/A'].shape[0]
                st.metric("Players with Instagram Data", f"{complete_data}/{len(players_df)}")
            with col4:
                with_search = players_df[players_df['google_search_volume'] > 0].shape[0]
                st.metric("Players with Search Data", f"{with_search}/{len(players_df)}")
            ...
            with col1:
                sport_filter = st.multiselect(
                    "Filter by Sport:",
                    options=['All'] + sorted(players_df['sport'].unique()),
                    default=['All']
                )
            ...
            with col3:
                show_complete_only = st.checkbox("Show only players with complete data", value=False)
            ...
            if show_complete_only:
                filtered_players = filtered_players[filtered_players['instagram_followers'] != 'N/A']
            ...
            st.warning("Player details data not available. Please check the JSON file.")
